...
%{
    ...
    % Usage case #1. Get default parameters
    noiseFreeComputeParams = nreNoiseFreeSceneAsResponses();
    ...
    if (length(sceneTemporalSupportSeconds) > 1)
        noiseFreeComputeParams.frameDurationSeconds = sceneTemporalSupportSeconds(2)-sceneTemporalSupportSeconds(1);
    end
    theNeuralEngine = neuralResponseEngine(@nreNoiseFreeSceneAsResponses,@nreNoisyInstancesPoisson,noiseFreeComputeParams,[]);
    ...
    % Create response instances with no noise another way, really just to show that we
    % can do this with the nreNoisyInstancesNoNoise compute function. It is
    % not clear that in practice you would ever want to do this.
    %
    % Note that we also produce noise free response instances with the version of
    % the nre that can add noise, by passing 'none' as the noiseFlag.
    theNeuralEngineNoNoise = neuralResponseEngine(@nreNoiseFreeSceneAsResponses,@nreNoisyInstancesNoNoise,noiseFreeComputeParams,[]);
