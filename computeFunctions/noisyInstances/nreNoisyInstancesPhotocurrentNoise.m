function dataOut = nreNoisyInstancesPhotocurrentNoise(...
...
%
% Syntax:
%   dataOut = nreNoisyInstancesPhotocurrentNoise(...
...
%    object.  This version adds photocurrent noise, and is intended to
%    operate on noise-free photocurrent responses
%
%
...
%       the Photocurrent noise model, based on the passed noise free photocurrent responses.
%
...
%    noiseFreeResponses  - a matrix of noise free neural (photocurrent)
...
%    07/28/2026  NPC  Wrote it.
    ...
    if (noiseFreeResponsesAreContrastResponses)
        error('Responses must be photocurrents')
    end
    ...
    switch (noiseFlag)
        case {'random'}
            ...
            for ii = 1:instancesNum
                parfor iCone = 1:responseDim
                    noiseFreePhotocurrentResponse = squeeze(noiseFreeResponses(noiseFreeInstancesNum, iCone,:));
                    noisyResponseInstances(ii,iCone,:) = osAddNoise(noiseFreePhotocurrentResponse, 'sampTime', dt);
                end
            end
        case {'none'}
            for ii = 1:instancesNum
