...
%}
    ...
    for idx = 1:length(noiseFlags)
        ...
        if (contains(ieParamFormat(noiseFlags{idx}), 'none'))
            ...
            [theConeMosaicResponses(noiseFlags{idx}), ~,~,~,temporalSupportSeconds] =...
            ...
            %there is one instance that excitationsToModulations is 0 for
            %all cones: when the passed stimulus is a null stimulus and we
            %assume no noise in cone excitations
            if ~any(modulations_cMosaicNoiseFree)
                modulations_cMosaicNoiseFree = modulations_cMosaicNoiseFree +...
                    randn(size(modulations_cMosaicNoiseFree)).*1e-6;
            end
        
            % Compute noise-free midget RGC response instances
            theNeuralResponses(noiseFlags{idx}) = ...
                theMidgetRGCmosaic.compute(...
                modulations_cMosaicNoiseFree, temporalSupportSeconds,...
                'nTrials', instancesNum, ...
                'seed', rngSeed);
        ...
        elseif (~isempty(rngSeed))
            ...
            % Set the theMidgetRGCmosaic.noiseFlag to 'none';
            theMidgetRGCmosaic.noiseFlag = 'random';
    ...
    % %visualize
    %cResp = theConeMosaicResponses(noiseFlags{idx});
    %theConeMosaic.visualize('activation',cResp(1,:,:));
    %mResp = theNeuralResponses(noiseFlags{idx});
    %theMidgetRGCmosaic.visualize('activation',mResp(1,:,:));
