...
%}
    ...
    if (isempty(neuralEngineOBJ.neuralPipeline))
        ...
        % Access the input cone mosaic that drives the mRGC mosaic
        % and set a different integration time
        theMidgetRGCmosaic.inputConeMosaic.integrationTime = 1000/1000;
    ...
    for idx = 1:length(noiseFlags)
        if (contains(ieParamFormat(noiseFlags{idx}), 'none'))
            ...
            theConeMosaicResponses(noiseFlags{idx}) = theConeMosaic.computeForOISequence(theOIsequence, ...
                'emPaths', emPaths, ...   % the emPaths
                'currentFlag', false ...  % no photocurrent
            );
        
            % Compute noise-free midget RGC response instances
            [theNeuralResponses(noiseFlags{idx}), temporalSupportSeconds] = theMidgetRGCmosaic.compute(...
                theConeMosaicResponses(noiseFlags{idx}), theConeMosaic.timeAxis);
        ...
        elseif (contains(ieParamFormat(noiseFlags{idx}), 'random'))
            ...
            % Compute noisy cone mosaic response instances
            theConeMosaicResponses(noiseFlags{idx}) = theConeMosaic.computeForOISequence(theOIsequence, ...
                'emPaths', emPaths, ...   % the emPaths
                'currentFlag', false, ...  % no photocurrent
            ...
            % Compute noisy midget RGC mosaic response instances
            [theNeuralResponses(noiseFlags{idx}), temporalSupportSeconds] = theMidgetRGCmosaic.compute( ...
                theConeMosaicResponses(noiseFlags{idx}), theConeMosaic.timeAxis, ...
                'seed', useSeed ...        % random seed
            );
