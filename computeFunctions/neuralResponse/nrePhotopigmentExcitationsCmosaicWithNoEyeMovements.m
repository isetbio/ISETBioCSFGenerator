...
%}
    ...
    boolean_callingMasterFunc = input(sprintf(['Warning: this function has been deprecated and ',...
        'replaced by nrePhotopigmentExcitationsCmosaic.m.\nIf you''d like to ',...
        'switch to the new function, type ''yes'', otherwise, type ''no'':']), ...
        's');

    %call the new master function
    if strcmp(boolean_callingMasterFunc,'yes')
        %before we do that, we need to do some sanity checks
        %1. check if all oisequence 
    %do it old way
    elseif strcmp(boolean_callingMasterFunc,'no')
