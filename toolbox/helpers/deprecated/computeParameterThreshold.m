...
% Compute threshold for a parameter (not necessarily contrast) in an N-alternative forced choice paradigm
%
% Syntax:
...
%  03/01/22  NPC     Wrote it by adapting computeThresholdNWay_OneStimulusPerTrial
    ...
    % Deprecated
    error('We think this function is no longer needed.  Change to ComputeThreshold.\n')
    ...
    while (nextFlag)
        ...
        if (isempty(testedIndex))
            ...
            eStart = tic;
            [predictions, tempClassifierEngine, responses] = computePerformanceNWay_OneStimulusPerTrial(...
