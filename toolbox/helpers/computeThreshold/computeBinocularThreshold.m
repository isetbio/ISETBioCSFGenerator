function [logThreshold, questObj, psychometricFunction, estimatorFittedPsychometricParams, ...
    ...
    trialByTrialWhichMetaDataLeftEye, trialByTrialWhichMetaDataRightEye] = computeBinocularThreshold(...
        theSceneEngine, theNeuralEngineLeftEye, theNeuralEngineRightEye, ...
        classifierEngine, classifierPara, thresholdPara, questEnginePara, varargin)
% Compute contrast threshold for different scenes, viewed through a binocular (left eye + right eye)
% neural response engine, and classifier engine. This function is suitable for both TAFC and 
% N-alternative forced-choice tasks. 
%
% Syntax:
%    [logThreshold, questObj, psychometricFunction, estimatorFittedPsychometricParams, ...
%    trialByTrialStimulusAlternatives, trialByTrialPerformance,triailByTrialWhichResponses, ...
%    trialByTrialWhichMetaDataLeftEye, trialByTrialWhichMetaDataRightEye] = ...
%    computeBinocularThreshold(theSceneEngine, theNeuralEngineLeftEye, theNeuralEngineRightEye, ...
...
%   theNeuralEngineLeftEye  - neuralResponseEngine object (left eye)
%   theNeuralEngineRightEye - neuralResponseEngine object (right eye)
...
p.addParameter('visualizeBinocularSummation', false, @islogical);
p.addParameter('thePDFrootDir', '', @ischar);
...
p.addParameter('binocularSummationPipelineVisualizedResponseRanges',  [], @(x)(isempty(x) || (isstruct(x))));
p.addParameter('nullStimulusInLeftEye', false, @islogical);
p.addParameter('nullStimulusInRightEye', false, @islogical);
...
thePDFrootDir = p.Results.thePDFrootDir;
...
binocularSummationPipelineVisualizedResponseRanges = p.Results.binocularSummationPipelineVisualizedResponseRanges;
nullStimulusInLeftEye = p.Results.nullStimulusInLeftEye;
nullStimulusInRightEye = p.Results.nullStimulusInRightEye;

if (nullStimulusInLeftEye) && (~nullStimulusInRightEye)
    stimulatedEye = 'RE';
elseif (~nullStimulusInLeftEye) && (nullStimulusInRightEye)
    stimulatedEye = 'LE';
elseif (~nullStimulusInLeftEye) && (~nullStimulusInRightEye)
    stimulatedEye = 'LE+RE';
else
...
if (iscell(theNeuralEngineLeftEye))
    if (~iscell(theNeuralEngineRightEye))
        error('inconsistent LE and RE neural engines')
    end
    assert(length(theNeuralEngineLeftEye) == length(theNeuralEngineRightEye), 'LE and RE neural engines do not consistent sizes');
    for nn = 1:length(theNeuralEngineLeftEye)
        responseVideoFileNameBaseLeftEye{nn} = theNeuralEngineLeftEye{nn}.responseVideoFileName;
        responseVideoFileNameBaseRightEye{nn} = theNeuralEngineRightEye{nn}.responseVideoFileName;
    end
else
    responseVideoFileNameBaseLeftEye = theNeuralEngineLeftEye.responseVideoFileName;
    responseVideoFileNameBaseRightEye = theNeuralEngineRightEye.responseVideoFileName;
end
...
trialByTrialWhichMetaDataLeftEye = containers.Map();
trialByTrialWhichMetaDataRightEye = containers.Map();

testCounter = 0;
stimCounter = 0;
whatIsThis = thresholdPara.maxParamValue;
...
while (nextFlag)
    ...
    if (isempty(testedIndex))
        ...
        if (stimCounter > maxVisualizedNoisyResponseInstanceStimuli)
            ...
            if (iscell(theNeuralEngineLeftEye))
                for nn = 1:length(theNeuralEngineLeftEye)
                    theNeuralEngineLeftEye{nn}.visualizeEachCompute = false;
                    theNeuralEngineRightEye{nn}.visualizeEachCompute = false;
                end
            else
                theNeuralEngineLeftEye.visualizeEachCompute = false;
                theNeuralEngineRightEye.visualizeEachCompute = false;
            end
            ...
            if (iscell(theNeuralEngineLeftEye))
                for nn = 1:length(theNeuralEngineLeftEye)
                    theNeuralEngineLeftEye{nn}.responseVideoFileName = [responseVideoFileNameBaseLeftEye{nn} sprintf('_stim%d',stimCounter)];
                    theNeuralEngineRightEye{nn}.responseVideoFileName = [responseVideoFileNameBaseRightEye{nn} sprintf('_stim%d',stimCounter)];
                end
            else
                theNeuralEngineLeftEye.responseVideoFileName = [responseVideoFileNameBaseLeftEye sprintf('_stim%d',stimCounter)];
                theNeuralEngineRightEye.responseVideoFileName = [responseVideoFileNameBaseRightEye sprintf('_stim%d',stimCounter)];
            end
        ...
        if (useMetaContrast)
            if ~isTAFC
                ...
                fprintf('\n\n\n----> [1] Computing (meta) scene with contrast %f\n\n', testContrast);
                [theSceneSequence, theSceneTemporalSupportSeconds] = ...
                    theSceneEngine.compute(testContrast);
                
                if (nullStimulusInLeftEye)
                   theLeftEyeSceneSequences{testedIndex} = theNullSceneSequence;
                else
                   theLeftEyeSceneSequences{testedIndex} = theSceneSequence;
                end

                if (nullStimulusInRightEye)
                   theRightEyeSceneSequences{testedIndex} = theNullSceneSequence;
                else
                   theRightEyeSceneSequences{testedIndex} = theSceneSequence;
                end

            else
                % TAFC: Generate the test scene; we have the null from outside the
                % trial loop above.
                fprintf('\n\n\n----> [2] Computing (meta) scene with contrast %f\n\n', testContrast)
                [theSceneSequence, theSceneTemporalSupportSeconds] = theSceneEngine.compute(testContrast);

                if (nullStimulusInLeftEye)
                    theLeftEyeSceneSequences{testedIndex} = {theNullSceneSequence, theNullSceneSequence};
                else
                    theLeftEyeSceneSequences{testedIndex} = {theNullSceneSequence, theSceneSequence};
                end

                if (nullStimulusInRightEye)
                    theRightEyeSceneSequences{testedIndex} = {theNullSceneSequence, theNullSceneSequence};
                else
                    theRightEyeSceneSequences{testedIndex} = {theNullSceneSequence, theSceneSequence};
                end
            ...
            if ~isTAFC
                % NWay: Generate the scenes for each alternative, at the test contrast
                % TAFC: Use the null scene and generate the test scene
                fprintf('\n\n\n----> [3] Computing (non-meta) scenes with contrast %f\n\n', testContrast)
                for oo = 1:length(theSceneEngine)
                    [theSceneSequence, theSceneTemporalSupportSeconds] = theSceneEngine{oo}.compute(testContrast);
                    if (nullStimulusInLeftEye)
                        theLeftEyeSceneSequences{testedIndex}{oo} = theNullSceneSequence;
                    else
                        theLeftEyeSceneSequences{testedIndex}{oo} = theSceneSequence;
                    end
                    if (nullStimulusInRightEye)
                        theRightEyeSceneSequences{testedIndex}{oo} = theNullSceneSequence;
                    else
                        theRightEyeSceneSequences{testedIndex}{oo} = theSceneSequence;
                    end
                ...
                % TAFC: Generate the test scene; we have the null from outside the
                % trial loop above.
                fprintf('\n\n\n----> [4] Computing (non-meta) scene with contrast %f\n\n', testContrast)
                [theSceneSequence, theSceneTemporalSupportSeconds] = theSceneEngine.compute(testContrast);
        ...
        if (p.Results.extraVerbose)
            ...
            if (nullStimulusInRightEye)
                theSceneSequence = theLeftEyeSceneSequences{testedIndex};
            else
                theSceneSequence = theRightEyeSceneSequences{testedIndex};
            end

            Wl_vec = theSceneSequence{theStim}{theFrame}.spectrum.wave;
            [~, index] = min(abs(Wl_vec - theWl));
            temp = theSceneSequence{theStim}{theFrame}.data.photons(:,:,index);
        ...
        if (visualizeStimulus)
            ...
            sE.visualizeSceneSequence(theSceneSequence{theStim},...
        ...
        for eeTrain = 1:nTrainFixationalEMPaths
            ...
            [~, tempTempClassifierEngine] = computeBinocularPerformance(...
                theLeftEyeSceneSequences{testedIndex}, theRightEyeSceneSequences{testedIndex}, theSceneTemporalSupportSeconds,...
                classifierPara.nTrain, 0, theNeuralEngineLeftEye, theNeuralEngineRightEye,...
                ...
                'visualizeBinocularSummation', visualizeBinocularSummation, ...
                'fixationalEM', trainFixationalEMObj, ...
                'useMetaContrast', useMetaContrast, ...
                'binocularSummationPipeline', binocularSummationPipeline, ...
                'conditionLabel', sprintf('train for EMpath %d, %s (%s)', eeTrain, contrastLabel, stimulatedEye), ...
                'thePDFrootDir', thePDFrootDir, ...
                'binocularSummationPipelineVisualizedResponseRanges',  binocularSummationPipelineVisualizedResponseRanges ...
                );
        ...
        if (nTrainFixationalEMPaths == 1 && nTestFixationalEMPaths == 1)
            [predictions, ~, ~, whichAlternatives, whichResponses, ...
             whichMetaDataLeftEye, whichMetaDataRightEye] = computeBinocularPerformance(...
                theLeftEyeSceneSequences{testedIndex}, theRightEyeSceneSequences{testedIndex}, ...
                theSceneTemporalSupportSeconds, 0, classifierPara.nTest, ...
                theNeuralEngineLeftEye, theNeuralEngineRightEye, theTrainedClassifierEngines{testedIndex}{1}, [], classifierPara.testFlag, ...
                'TAFC', isTAFC, ...
                'saveResponses', false, ...
                'visualizeAllComponents', visualizeAllComponents, ...
                'visualizeBinocularSummation', visualizeBinocularSummation, ...
                ...
                'useMetaContrast', useMetaContrast, ...
                'binocularSummationPipeline', binocularSummationPipeline, ...
                'conditionLabel', sprintf('predict, %s (%s)', contrastLabel, stimulatedEye), ...
                'thePDFrootDir', thePDFrootDir, ...
                'binocularSummationPipelineVisualizedResponseRanges', binocularSummationPipelineVisualizedResponseRanges ...
                );
        ...
        elseif (nTrainFixationalEMPaths == 1 && nTestFixationalEMPaths > 1)
            ...
            whichAlternatives = [];
            whichResponses = [];
            whichMetaDataLeftEye = {};
            whichMetaDataRightEye = {};
            ...
            for eeTest = 1:nTestFixationalEMPaths
                ...
                % Predict
                [predictionsTemp, ~, ~, whichAlternativesTemp, whichResponsesTemp, ...
                 whichMetaDataTempLeftEye, whichMetaDataTempRightEye] = computeBinocularPerformance(...
                    theLeftEyeSceneSequences{testedIndex}, theRightEyeSceneSequences{testedIndex}, ...
                    theSceneTemporalSupportSeconds, 0, nTestPerEMPath, ...
                    theNeuralEngineLeftEye, theNeuralEngineRightEye, ...
                    theTrainedClassifierEngines{testedIndex}{1}, [], classifierPara.testFlag, ...
                    'TAFC', isTAFC, ...
                    'saveResponses', false, ...
                    'visualizeAllComponents', visualizeAllComponents, ...
                    'visualizeBinocularSummation', visualizeBinocularSummation, ...
                    ...
                    'useMetaContrast', useMetaContrast, ...
                    'binocularSummationPipeline', binocularSummationPipeline, ...
                    'conditionLabel', sprintf('predict for EMpath %d, %s (%s)', eeTest, contrastLabel, stimulatedEye), ...
                    'thePDFrootDir', thePDFrootDir, ...
                   'binocularSummationPipelineVisualizedResponseRanges', binocularSummationPipelineVisualizedResponseRanges ...
                );
                ...
                whichMetaDataLeftEye{numel(whichMetaDataLeftEye)+1} = whichMetaDataTempLeftEye;
                whichMetaDataRightEye{numel(whichMetaDataRightEye)+1} = whichMetaDataTempRightEye;
            end
            ...
            for eeTrain = 1:nTrainFixationalEMPaths
                ...
                [predictionsTemp, ~, ~, whichAlternativesTemp, whichResponsesTemp, ...
                 whichMetaDataTempLeftEye, whichMetaDataTempRightEye] = computeBinocularPerformance(...
                    ...
                    theSceneTemporalSupportSeconds, 0, nTestPerEMPath, ...
                    theNeuralEngineLeftEye, theNeuralEngineRightEye, ...
                    theTrainedClassifierEngines{testedIndex}{eeTrain}, [], classifierPara.testFlag, ...
                    'TAFC', isTAFC, ...
                    'saveResponses', false, ...
                    'visualizeAllComponents', visualizeAllComponents, ...
                    'visualizeBinocularSummation', visualizeBinocularSummation, ...
                    ...
                    'useMetaContrast', useMetaContrast, ...
                    'binocularSummationPipeline', binocularSummationPipeline, ...
                    'conditionLabel', sprintf('predict for EMpath %d, %s (%s)', eeTrain, contrastLabel, stimulatedEye), ...
                    'thePDFrootDir', thePDFrootDir, ...
                    'binocularSummationPipelineVisualizedResponseRanges', binocularSummationPipelineVisualizedResponseRanges ...
                    );
        ...
        trialByTrialWhichMetaDataLeftEye(contrastLabel) = whichMetaDataLeftEye;
        trialByTrialWhichMetaDataRightEye(contrastLabel) = whichMetaDataRightEye;
        ...
        if (datasavePara.saveMRGCResponses)
            theMRGCmosaicLeftEye = theNeuralEngineLeftEye.neuralPipeline.mRGCmosaic;
            theMRGCmosaicRightEye = theNeuralEngineRightEye.neuralPipeline.mRGCmosaic;
            ...
            if (neuralEngineSaved == false)
                ...
                % LEFT EYE
                mosaicFileName = fullfile(datasavePara.destDir,'mRGCmosaicLeftEye.mat');
                fprintf('Saving mRGC mosaic to %s.\n', mosaicFileName);
                save(mosaicFileName, 'theMRGCmosaicLeftEye', '-v7.3');
                % RIGHT EYE
                mosaicFileName = fullfile(datasavePara.destDir,'mRGCmosaicRightEye.mat');
                fprintf('Saving mRGC mosaic to %s.\n', mosaicFileName);
                save(mosaicFileName, 'theMRGCmosaicRightEye', '-v7.3');
        ...
        if iscell(theNeuralEngineLeftEye)
            for n = 1:length(theNeuralEngineLeftEye)
                if n == 1
                    savevisualizeEachComputeLeftEye = theNeuralEngineLeftEye{n}.visualizeEachCompute;
                    savevisualizeEachComputeRightEye = theNeuralEngineRightEye{n}.visualizeEachCompute;
                end
                theNeuralEngineLeftEye{n}.visualizeEachCompute = false;
                theNeuralEngineRightEye{n}.visualizeEachCompute = false;
            end
        elseif isstruct(theNeuralEngineLeftEye)
            savevisualizeEachComputeLeftEye = theNeuralEngineLeftEye.visualizeEachCompute;
            savevisualizeEachComputeRightEye = theNeuralEngineRightEye.visualizeEachCompute;
            theNeuralEngineLeftEye.visualizeEachCompute = false;
            theNeuralEngineRightEye.visualizeEachCompute = false;
        end
            ...
            for eeTest = 1:nTestFixationalEMPaths
                ...
                [predictionsTemp, ~, ~, whichAlternativesTemp, whichResponsesTemp, ...
                 whichMetaDataTempLeftEye, whichMetaDataTempRightEye] = computeBinocularPerformance(...
                    theLeftEyeSceneSequences{testedIndex}, theRightEyeSceneSequences{testedIndex}, ...
                    theSceneTemporalSupportSeconds, 0, classifierPara.nTest, ...
                    theNeuralEngineLeftEye, theNeuralEngineRightEye, theTrainedClassifierEngines{testedIndex}{eeTrain}, [], classifierPara.testFlag, ...
                    'TAFC', isTAFC, ...
                    'saveResponses', false, ...
                    'visualizeAllComponents', visualizeAllComponents, ...
                    'visualizeBinocularSummation', visualizeBinocularSummation, ...
        ...
        if iscell(theNeuralEngineLeftEye)
            for n = 1:length(theNeuralEngineLeftEye)
                theNeuralEngineLeftEye{n}.visualizeEachCompute = savevisualizeEachComputeLeftEye;
                theNeuralEngineRightEye{n}.visualizeEachCompute = savevisualizeEachComputeRightEye;
            end
        elseif isstruct(theNeuralEngineLeftEye)
            theNeuralEngineLeftEye.visualizeEachCompute = savevisualizeEachComputeLeftEye;
            theNeuralEngineRightEye.visualizeEachCompute = savevisualizeEachComputeRightEye;
        end
        ...
        prevTemp = trialByTrialWhichMetaDataLeftEye(contrastLabel);
        prevTemp{numel(prevTemp)+1} = whichMetaDataLeftEye;
        trialByTrialWhichMetaDataLeftEye(contrastLabel) = prevTemp;

        prevTemp = trialByTrialWhichMetaDataRightEye(contrastLabel);
        prevTemp{numel(prevTemp)+1} = whichMetaDataRightEye;
        trialByTrialWhichMetaDataRightEye(contrastLabel) = prevTemp;
