function [logThreshold, questObj, psychometricFunction, estimatorFittedPsychometricParams, ...
    trialByTrialStimulusAlternatives,trialByTrialPerformance,triailByTrialWhichResponses, trialByTrialWhichMetaData] = ...
...
trialByTrialWhichMetaData = containers.Map();
...
while (nextFlag)
    ...
    if (isempty(testedIndex))
        ...
        if (nTrainFixationalEMPaths == 1 && nTestFixationalEMPaths == 1)
            [predictions, ~, ~, whichAlternatives, whichResponses, whichMetaData] = computePerformance(theSceneSequences{testedIndex}, ...
        ...
        elseif (nTrainFixationalEMPaths == 1 && nTestFixationalEMPaths > 1)
            ...
            for eeTest = 1:nTestFixationalEMPaths
                ...
                % Predict
                [predictionsTemp, ~, ~, whichAlternativesTemp, whichResponsesTemp, whichMetaDataTemp] = computePerformance(theSceneSequences{testedIndex}, ...
                ...
                whichMetaData{numel(whichMetaData)+1} = whichMetaDataTemp;
            end
        ...
        trialByTrialWhichMetaData(contrastLabel) = whichMetaData;
        ...
        prevTemp = trialByTrialWhichMetaData(contrastLabel);
        prevTemp{numel(prevTemp)+1} = whichMetaData;
        trialByTrialWhichMetaData(contrastLabel) = prevTemp;
