...
%                   functions.
    ...
    if isfield(theNeuralEngine.neuralPipeline, 'mRGCmosaic')
        theNeuralEngineMosaic = theNeuralEngine.neuralPipeline.mRGCmosaic;
        rfPos = theNeuralEngineMosaic.rgcRFpositionsDegs;
    elseif isfield(theNeuralEngine.neuralPipeline, 'coneMosaic')
        theNeuralEngineMosaic = theNeuralEngine.neuralPipeline.coneMosaic;
        rfPos = theNeuralEngineMosaic.coneRFpositionsDegs;
    end
    xq = rfPos(:,1);
    xq = xq-mean(xq(:));
    yq = rfPos(:,2);
...
%             title(sprintf('quadrature, tBin = %d', tBin));
            ...
            ax = subplot(1,2,1);
            theNeuralEngineMosaic.visualize(...
                'figureHandle', hFig, 'axesHandle', ax, ...
                'activation', poolingWeights(1,tBin,:), ...
                'activationRange', activationRange, ... %'crossHairsOnMosaicCenter', true, ...
