...
%{
    ...
    validationThresholds = [0.3230    0.1876    0.1517    0.1564];
...
switch (whichNoiseFreeNre)
    ...
    case 'mRGCMosaic'
        ...
        for iName = 1:numel(synthesizedRGCmosaicNames)
            tmpPstruct = RGCMosaicConstructor.helper.utils.initializeRGCMosaicGenerationParameters('human', 'JCNpaperDefaultSubject', synthesizedRGCmosaicNames{iName}, []);
            if (...
...
if (isempty(options.psychometricCurveSamplesNum))
    ...
    questEnginePara = struct( ...
        'qpPF',@qpPFWeibullLog, ...
        'minTrial', nTest*options.psychometricCurveSamplesNum, ...
        'maxTrial', nTest*options.psychometricCurveSamplesNum, ...
        'numEstimator', 1, ...
        'stopCriterion', 0.05);
end
