function t_WatsonTemporalFilterWithEyeMovements(options)
% Apply Watson's temporal filter to a cone mosaic excitation sequence
%
% Syntax:
%    t_WatsonTemporalFilterWithEyeMovements
%
% Description:
%    Demonstrates how to generate an expanding bulleye stimulus, deliver it
%    to a cone mosaic under physiological optics, with or without fixational 
%    eye movements, compute the cone mosaic excitations and then apply Watson's 
%    temporal filter.
...
%    - 'fastParameters':                       logical, whether to use params that make the simuation run fast (but low resolution)
%    - 'fixationalEyeMovements':               logical, whether to include fixational eye movements    
%    - 'reformatExportedAVIvideoToMP4format':  logical, whether to reformat the exported AVI videos to MP4 format
%    - 'loadResponsesFromPreviousSimulation':  logical, whether to load responses already computed 
%
%
% See Also:
...
%}
    ...
    if (fixationalEyeMovements)
        simFileName = sprintf('WatsonTemporalFilterWithFixationalEM.mat');
    else
        simFileName = sprintf('WatsonTemporalFilterWithoutFixationalEM.mat');
    end
    ...
    if (loadResponsesFromPreviousSimulation)
        ...
        load(simFileName, ...
            ...
            'theWatsonFilterImpulseResponseStruct', ...
            'responseTimeAxis', ...
            'coneMosaicWatsonFilterSpatiotemporalActivation', ...
            'coneMosaicNoisyWatsonFilterSpatiotemporalActivation', ...
    ...
    mosaicEccDegs = [0 0];
    mosaicIntegrationTimeSeconds = 5/1000;
    ...
    % Compute the Watson impulse response 
    theWatsonFilterImpulseResponseStruct = struct();
    theWatsonFilterImpulseResponseStruct.filter = WatsonFilter([],theOIsequence.timeAxis);
    theWatsonFilterImpulseResponseStruct.temporalSupportSeconds = theOIsequence.timeAxis;

    tBinsWatsonFilterResponse = size(coneMosaicSpatiotemporalActivation,2) + numel(theWatsonFilterImpulseResponseStruct.temporalSupportSeconds) - 1;
    coneMosaicWatsonFilterSpatiotemporalActivation = zeros(nTrials, tBinsWatsonFilterResponse, theConeMosaic.conesNum);
   
    % Compute Watson filter responses
    for coneIndex = 1:theConeMosaic.conesNum
        for iTrial = 1:nTrials
             ...
             theWatsonFilterResponse = ...
                 conv(theConeExcitationResponseTimeSeries, theWatsonFilterImpulseResponseStruct.filter);
     
             % Retrieve the part of the response following the inserted null cone excitation response
             idx = size(coneMosaicNullResponse,2)+1 : numel(theWatsonFilterResponse);
             coneMosaicWatsonFilterSpatiotemporalActivation(iTrial, :, coneIndex) = theWatsonFilterResponse(idx);
        end % iTrial
    end % parfor coneIndex

    % Keep the part of the Watson filter response that extends over the temporal extent of the cone mosaic excitation 
    tBins = 1:size(coneMosaicSpatiotemporalActivation,2);
    coneMosaicWatsonFilterSpatiotemporalActivation = coneMosaicWatsonFilterSpatiotemporalActivation(:,tBins,:);
    
    % Add post-filter noise (at the moment, zero noise)
    coneMosaicNoisyWatsonFilterSpatiotemporalActivation = coneMosaicWatsonFilterSpatiotemporalActivation;
...
function generateResponseVideos(responseTimeAxis, ...
    ...
    theResponses = coneMosaicNoisyWatsonFilterSpatiotemporalActivation(:,:,[theLconeIndex theMconeIndex theSconeIndex]);
    WatsonFilterResponseRange = [min(theResponses(:)) max(theResponses(:))]; 
    ...
    for iTrial = 1:nTrials
        ...
        coneMosaicWatsonFilterResponseTimeAxis = (0:(size(coneMosaicWatsonFilterSpatiotemporalActivation,2)-1))*dT;

        
        % The noisy Watson filter responses
        plot(ax,coneMosaicWatsonFilterResponseTimeAxis*1e3, squeeze(coneMosaicNoisyWatsonFilterSpatiotemporalActivation(iTrial,:,theLconeIndex)), '-', 'Color', 'r', 'LineWidth', 1);
        hold(ax, 'on')
        plot(ax,coneMosaicWatsonFilterResponseTimeAxis*1e3, squeeze(coneMosaicNoisyWatsonFilterSpatiotemporalActivation(iTrial,:,theMconeIndex)), '-', 'Color', 'g', 'LineWidth', 1);
        plot(ax,coneMosaicWatsonFilterResponseTimeAxis*1e3, squeeze(coneMosaicNoisyWatsonFilterSpatiotemporalActivation(iTrial,:,theSconeIndex)), '-', 'Color', 'b', 'LineWidth', 1);
        
        % The noise-free Watson filter responses
        plot(ax,coneMosaicWatsonFilterResponseTimeAxis*1e3, squeeze(coneMosaicWatsonFilterSpatiotemporalActivation(iTrial,:,theLconeIndex)), '-', 'Color', 'k', 'LineWidth', 4);
        plot(ax,coneMosaicWatsonFilterResponseTimeAxis*1e3, squeeze(coneMosaicWatsonFilterSpatiotemporalActivation(iTrial,:,theMconeIndex)), '-', 'Color', 'k', 'LineWidth', 4);
        plot(ax,coneMosaicWatsonFilterResponseTimeAxis*1e3, squeeze(coneMosaicWatsonFilterSpatiotemporalActivation(iTrial,:,theSconeIndex)), '-', 'Color', 'k', 'LineWidth', 4);
        
        p1 = plot(ax,coneMosaicWatsonFilterResponseTimeAxis*1e3, squeeze(coneMosaicWatsonFilterSpatiotemporalActivation(iTrial,:,theLconeIndex)), '-', 'Color', 'r', 'LineWidth', 1.5);
        p2 = plot(ax,coneMosaicWatsonFilterResponseTimeAxis*1e3, squeeze(coneMosaicWatsonFilterSpatiotemporalActivation(iTrial,:,theMconeIndex)), '-', 'Color', 'g', 'LineWidth', 1.5);
        p3 = plot(ax,coneMosaicWatsonFilterResponseTimeAxis*1e3, squeeze(coneMosaicWatsonFilterSpatiotemporalActivation(iTrial,:,theSconeIndex)), '-', 'Color', 'b', 'LineWidth', 1.5);
        legend(ax, [p1 p2 p3], {'L-cone', 'M-cone', 'S-cone'}, 'Location', 'NorthWest');
        xlabel(ax,'time (msec)');
        ylabel(ax, '');
        %set(ax, 'YTickLabel', {});
        ...
        grid(ax, 'on');
        set(ax, 'XLim', [responseTimeAxis(1) responseTimeAxis(end)]*1e3, 'FontSize', fontSize);
        set(ax, 'YLim', WatsonFilterResponseRange);
        ylabel(ax, 'Watson filter response');

        % Plot the pCurrent impulse response
        ax = axes('Position', [0.72 0.43 0.25 0.1]);
        plot(ax, theWatsonFilterImpulseResponseStruct.temporalSupportSeconds*1e3, theWatsonFilterImpulseResponseStruct.filter, 'k-', 'LineWidth', 4);
        hold on;
        plot(ax, theWatsonFilterImpulseResponseStruct.temporalSupportSeconds*1e3, theWatsonFilterImpulseResponseStruct.filter, 'r-', 'LineWidth', 1.5);

        ylabel(ax, 'Watson filter IR');
        set(ax, 'fontSize', 12);
        box(ax, 'off');
        grid(ax, 'on')
        ...
        if (fixationalEyeMovements)
            ...
            NicePlot.exportFigToPDF(sprintf('%s/responseTimeSeries.pdf',figureFileBase), hFig, 300);
        end
    end
    ...
    WatsonFilterResponseRange = [min(coneMosaicWatsonFilterSpatiotemporalActivation(:)) max(coneMosaicWatsonFilterSpatiotemporalActivation(:))];     
    ...
    for iTrial = 1:size(coneMosaicNoisySpatiotemporalActivation,1)
        ...
        if (fixationalEyeMovements)
            ...
            theVideoFileName2 = sprintf('%s/NoiseFreeWatsonFilterTrial%d',figureFileBase, iTrial);
            ...
            theVideoFileName4 = sprintf('%s/NoisyWatsonFilterTrial%d',figureFileBase, iTrial);
            ...
            theVideoFileName1 = sprintf('%s/NoiseFreeExcitations',figureFileBase);
	        %videoOBJ1 = VideoWriter(theVideoFileName1, 'M PEG-4');  % H264format (has artifacts)
            videoOBJ1 = VideoWriter(theVideoFileName1, 'Uncompressed AVI');
	        videoOBJ1.FrameRate = 30;
	        %videoOBJ1.Quality = 100;
            ...
            theVideoFileName2 = sprintf('%s/NoiseFreeWatsonFilter',figureFileBase);
	        % videoOBJ2 = VideoWriter(theVideoFileName2, 'MPEG-4'); % H264format (has artifacts)
            videoOBJ2 = VideoWriter(theVideoFileName2, 'Uncompressed AVI');
	        videoOBJ2.FrameRate = 30;
	        %videoOBJ2.Quality = 100;
            ...
            theVideoFileName3 = sprintf('%s/NoisyExcitations',figureFileBase);
	        % videoOBJ3 = VideoWriter(theVideoFileName3, 'MPEG-4'); % H264format (has artifacts)
            videoOBJ3 = VideoWriter(theVideoFileName3, 'Uncompressed AVI');
	        videoOBJ3.FrameRate = 30;
	        %videoOBJ3.Quality = 100;
            ...
            theVideoFileName4 = sprintf('%s/NoisyWatsonFilter',figureFileBase);
	        % videoOBJ4 = VideoWriter(theVideoFileName3, 'MPEG-4'); % H264format (has artifacts)
            videoOBJ4 = VideoWriter(theVideoFileName4, 'Uncompressed AVI');
	        videoOBJ4.FrameRate = 30;
	        %videoOBJ4.Quality = 100;
        ...
        for t = 1:size(coneMosaicNoisySpatiotemporalActivation,2)
            ...
            if (fixationalEyeMovements)
                ...
                theConeMosaic.visualize('figureHandle', hFig2, 'axesHandle', ax2, ...
                    'activation', squeeze(coneMosaicWatsonFilterSpatiotemporalActivation(iTrial,t,:)), ...
                    'activationRange', WatsonFilterResponseRange, ...
                    'domainvisualizationlimits', domainvisualizationlimits, ...
                    'labelConesWithIndices', theExemplarConeIndices, ...
                    'displayedEyeMovementData', struct(...
                        'trial', iTrial, ...
                    ...
                    'backgroundColor', backgroundColor, ...
                    'fontSize', fontSize, ...
                    'plotTitle', sprintf('noise-free Watson filter response\nt = %2.0f msec', 1000*responseTimeAxis(t)));
            else
                % Visualize cone mosaic response without fEM
                theConeMosaic.visualize('figureHandle', hFig2, 'axesHandle', ax2, ...
                    'activation', squeeze(coneMosaicWatsonFilterSpatiotemporalActivation(iTrial,t,:)), ...
            ...
            if (fixationalEyeMovements)
                ...
                theConeMosaic.visualize('figureHandle', hFig4, 'axesHandle', ax4, ...
                    'activation', squeeze(coneMosaicNoisyWatsonFilterSpatiotemporalActivation(iTrial,t,:)), ...
                    'activationRange', WatsonFilterResponseRange, ...
                    'domainvisualizationlimits', domainvisualizationlimits, ...
                    'labelConesWithIndices', theExemplarConeIndices, ...
                    'displayedEyeMovementData', struct(...
                    ...
                    'backgroundColor', backgroundColor, ...
                    'fontSize', fontSize, ...
                    'plotTitle', sprintf('noisy Watson filter response\nt = %2.0f msec', 1000*responseTimeAxis(t)));
            else
                % Visualize cone mosaic response without fEM
                theConeMosaic.visualize('figureHandle', hFig4, 'axesHandle', ax4, ...
                    'activation', squeeze(coneMosaicNoisyWatsonFilterSpatiotemporalActivation(iTrial,t,:)), ...
