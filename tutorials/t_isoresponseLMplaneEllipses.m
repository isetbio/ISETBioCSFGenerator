...
arguments
    ...
    options.synthesizedRGCmosaicName (1,:) char = 'JCNpaperFovealMosaic';
        ...
        for i = 1:numel(theKeys)
            ...
            for jj = 1:numel(ww)
                ...
                if (isstruct(theMetaData)) && (isfield(theMetaData, 'mRGCnonLinearityFigureHandle'))
                    theMetaData.mRGCnonLinearityFigureHandle
                    if (~isempty(theMetaData.mRGCnonLinearityFigureHandle))
                        hFig = figure(theMetaData.mRGCnonLinearityFigureHandle);
                    else
                        hFig = figure(randi(10000));
                    end
...
function [theBackgroundSceneEngine, theNullSceneEngine, theTestSceneEngines] = configureStimulusSceneEngines(...
    ...
    elseif (isfield(gratingSceneParams, 'backgroundLMSconeExcitations'))
        gratingSceneParamsForNullScene.sceneEngineName = ...
            sprintf('null stimulus engine (reference contrast: %2.3f %2.3f %2.3f)', ...
                referenceLMSconeContrast(1), referenceLMSconeContrast(2), referenceLMSconeContrast(3));

        % Adjust the background LMS excitations if we get passed a
        % referenceLMSconeContrast and backgroundLMSconeExcitations
    ...
    if (~isempty(referenceLMSconeContrast)) && (isfield(gratingSceneParams, 'backgroundLMSconeExcitations'))
        % Adjust the background LMS excitations if we get passed a
        % referenceLMSconeContrast and backgroundLMSconeExcitations
