function t_rasterAOSLO_FullBiophysicalOS
    ...
    if (recomputePhotocurrents)
        ...
        warmUpPeriodSecondsToRetain = stimulusRefreshIntervalSeconds*10;
        ...
        if (testIncrementDecrementScenes)
            ...
            [photocurrentResponseDecrementsTimeSeries, photocurrentResponseTimeAxis, photocurrentResponseDecrementsTimeSeriesNoisy] = ...
                computeConeMosaicPhotoCurrentsResponse(...
                    ...
                    backgroundConeExcitationResponseTimeSeries, ...
                    noiseFreeConeExcitationDecrementsResponseTimeSeries, ...
                    warmupPeriodStimulusIntervalsNum, warmUpPeriodSecondsToRetain, ...
    ...
    for iTrial = 1:nTrials
        for iTimePoint = 1:size(LconeIndicesResponses,2) 
            ...
            if (~strcmp(signalType, 'excitations'))
                ...
                if (currentTime < minNonCausalDelay)
                    continue;
                end
            ...
            % Determined displayed time indices
            idx = find(responseTimeAxis>=minNonCausalDelay & responseTimeAxis<=currentTime);
            ...
            hold(ax3, 'off');
            set(ax3, 'XLim', [minNonCausalDelay responseTimeAxis(end)]*1000, 'XTick', -300:10:500, 'YLim', activationRange, 'YTick', -100:5:0);
...
function [photocurrentResponseTimeSeries, photocurrentResponseTimeAxis, photocurrentResponseTimeSeriesNoisy] = ...
    ...
    warmUpSamplesToIncludeInVisualization = ceil(warmUpPeriodSecondsToRetain / theConeMosaic.integrationTime);
    actualwarmUpPeriodSecondsToRetain = warmUpSamplesToIncludeInVisualization * theConeMosaic.integrationTime;
    ...
    [photocurrentResponseTimeSeries, photocurrentResponseTimeAxis, photocurrentResponseTimeSeriesNoisy] = ...
         ...
         theConeMosaic.integrationTime, warmupPeriodSeconds, actualwarmUpPeriodSecondsToRetain, ...
         subtractBackgroundPhotoCurrents, debugWarmUpTime);
end
...
function [photocurrentResponseTimeSeries, photocurrentResponseTimeAxis, photocurrentResponseTimeSeriesNoisy] = ...
    ...
    coneMosaicIntegrationTime, warmupPeriodSeconds, warmUpPeriodSecondsToRetain, ...
    subtractBackgroundPhotoCurrents, debugWarmUpTime)
    ...
    parfor iCone = 1:nCones
        ...
        fprintf('Computing pCurrent responses cone %d (type:%d) of %d (%d fEM paths)\n', ...
            iCone, theConeMosaic.coneTypes(iCone), nCones, nTrials);
        ...
        for iTrial = 1:nTrials
            ...
            singleConeSingleTrialConeExcitationsCountResponse = ...
                qinterp1(tIn, singleConeSingleTrialConeExcitationsCountResponse, tOut, 0);
            ...
            %fprintf('pCurrent response for fEM path #%d - cone %d (type:%d) of %d: range: [%2.2f %2.2f]\n', iTrial, iCone, theConeMosaic.coneTypes(iCone), nCones, min(theOSphotoCurrentResponse), max(theOSphotoCurrentResponse));
    ...
    % Only keep response at t >= -warmUpPeriodSecondsToRetain;
    if (warmupPeriodSeconds > 0)
        ...
        idx = find(photocurrentResponseTimeAxis >= -warmUpPeriodSecondsToRetain);
