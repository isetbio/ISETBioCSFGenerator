...
%}
   ...
   % Override parameters if we want to go through the paces fast
    ...
    tBinsPhotocurrentResponse = size(coneMosaicSpatiotemporalActivation,2) + size(coneMosaicNullResponse,2) + numel(thePhotocurrentImpulseResponseStruct.temporalSupportSeconds) - 1;
    ...
    for coneIndex = 1:theConeMosaic.conesNum
        ...
        for iTrial = 1:nTrials
            ...
            % Insert the null cone excitation differential response (which is 0) at the end
            theConeExcitationResponseTimeSeries = ...
                cat(2,theConeExcitationResponseTimeSeries, 0*coneMosaicNullResponse(1,:, coneIndex));
    ...
    tBins = 1:(size(coneMosaicSpatiotemporalActivation,2)+size(coneMosaicNullResponse,2));
    ...
    responseTimeAxis = (0:(numel(tBins)-1))*dT;
...
function generateResponseVideos(responseTimeAxis, ...
    ...
    size(responseTimeAxis)
    coneExcitationsResponseTimeAxis = responseTimeAxis(1:size(coneMosaicNoisySpatiotemporalActivation,2));
    size(coneExcitationsResponseTimeAxis)
    size(coneMosaicNoisySpatiotemporalActivation,2)
    for iTrial = 1:nTrials
        ...
        bar(ax,coneExcitationsResponseTimeAxis*1e3, squeeze(coneMosaicNoisySpatiotemporalActivation(:,:,theLconeIndex))/theConeMosaic.integrationTime,  1, 'EdgeColor', [1 0 0],   'FaceColor', [1 0.5 0.5],   'FaceAlpha', 0.5, 'EdgeAlpha', 0.5, 'LineWidth', 0.5);
        hold(ax, 'on')
        bar(ax,coneExcitationsResponseTimeAxis*1e3, squeeze(coneMosaicNoisySpatiotemporalActivation(:,:,theMconeIndex))/theConeMosaic.integrationTime, 1, 'EdgeColor', [0 0.8 0], 'FaceColor', [0.5 0.8 0.5], 'FaceAlpha', 0.5, 'EdgeAlpha', 0.5, 'LineWidth', 0.5);
        bar(ax,coneExcitationsResponseTimeAxis*1e3, squeeze(coneMosaicNoisySpatiotemporalActivation(:,:,theSconeIndex))/theConeMosaic.integrationTime, 1, 'EdgeColor', [0 0 1],   'FaceColor', [0.5 0.5 1],   'FaceAlpha', 0.5, 'EdgeAlpha', 0.5, 'LineWidth', 0.5);
        
        % The noise-free excitations
        plot(ax,coneExcitationsResponseTimeAxis*1e3, squeeze(coneMosaicSpatiotemporalActivation(iTrial,:,theLconeIndex))/theConeMosaic.integrationTime, '-', 'Color', 'k', 'LineWidth', 4);
        plot(ax,coneExcitationsResponseTimeAxis*1e3, squeeze(coneMosaicSpatiotemporalActivation(iTrial,:,theMconeIndex))/theConeMosaic.integrationTime, '-', 'Color', 'k', 'LineWidth', 4);
        plot(ax,coneExcitationsResponseTimeAxis*1e3, squeeze(coneMosaicSpatiotemporalActivation(iTrial,:,theSconeIndex))/theConeMosaic.integrationTime, '-', 'Color', 'k', 'LineWidth', 4);
       
        p1 = plot(ax,coneExcitationsResponseTimeAxis*1e3, squeeze(coneMosaicSpatiotemporalActivation(iTrial,:,theLconeIndex))/theConeMosaic.integrationTime, '-', 'Color', 'r', 'LineWidth', 1.5);
        p2 = plot(ax,coneExcitationsResponseTimeAxis*1e3, squeeze(coneMosaicSpatiotemporalActivation(iTrial,:,theMconeIndex))/theConeMosaic.integrationTime, '-', 'Color', [0 0.8 0], 'LineWidth', 1.5);
        p3 = plot(ax,coneExcitationsResponseTimeAxis*1e3, squeeze(coneMosaicSpatiotemporalActivation(iTrial,:,theSconeIndex))/theConeMosaic.integrationTime, '-', 'Color', 'b', 'LineWidth', 1.5);
        ...
        dT = responseTimeAxis(2)-responseTimeAxis(1);
        coneMosaicPhotocurrentResponseTimeAxis = responseTimeAxis;
    ...
    for iTrial = 1:size(coneMosaicNoisySpatiotemporalActivation,1)
        ...
        % Render video frames
        photocurrentResponseBinsNum = size(coneMosaicPhotocurrentSpatiotemporalActivation,2);
        coneExcitationsResponseBinsNum = size(coneMosaicNoisySpatiotemporalActivation,2);
        for t = 1:photocurrentResponseBinsNum

            if (t <= coneExcitationsResponseBinsNum)
                if (fixationalEyeMovements)
            ...
            if (fixationalEyeMovements) && (t <= coneExcitationsResponseBinsNum)
                % Visualize cone mosaic response with fEM
                theConeMosaic.visualize('figureHandle', hFig2, 'axesHandle', ax2, ...
                    ...
                    'displayedEyeMovementData', struct(...
                        'trial', iTrial, ...
                        'timePoints', 1:min([t coneExcitationsResponseBinsNum])), ...
                    'horizontalActivationColorbar', true, ...
                    'colorbarTickLabelColor', colorbarTickLabelColor, ...
                    'backgroundColor', backgroundColor, ...
                    'fontSize', fontSize, ...
